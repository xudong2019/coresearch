...
plt.plot(np.mean(rc,axis=0),'b-', marker="o", linewidth=0.25,markersize=0.25)
plt.plot(abs(s), 'k',  linewidth=0.25, markersize=0.25)
legends.append('分钟价格回报率变化图')
legends.append('分钟价格回报率置信度（回报率/方差）')
...
def getPnl():
    ...
    for (i,t) in enumerate(trades):
        ...
        q1 = dbt.minuteBarStock.find_one({'ticker':tkrs[list(name).index(n)], 'dateTime':dtIn},{'ticker':1, 'name':1, 'open':1})
        q2 = dbt.minuteBarStock.find_one({'ticker':tkrs[list(name).index(n)], 'dateTime':dtOt},{'ticker':1, 'name':1, 'open':1})
        if q1==None:
            continue
        if q2==None:
            continue
...
get_ipython().run_line_magic('prun', 'getPnl()')
...
get_ipython().run_line_magic('load_ext', 'line_profiler')
get_ipython().run_line_magic('lprun', '-f getPnl getPnl()')
